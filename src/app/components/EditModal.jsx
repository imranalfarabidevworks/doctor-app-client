...
export default function EditModal({ booking }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/appointments/${booking._id}`, {
        method: "PUT",
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border rounded-xl px-4 py-2 mt-1 bg-transparent outline-none"
                  required
                />
              </div>
              ...
              <div className="flex gap-3 mt-2">
                <button
                  ...
                  className="flex-1 border rounded-xl py-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-xl py-2 transition"
                >
